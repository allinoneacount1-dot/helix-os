    ...
    <div className="flex items-end gap-0.5 h-10">
      {data.map((val, i) => (
        <motion.div
          ...
          animate={{ height: `${((val - min) / range) * 100}%`, opacity: 0.3 + (i / data.length) * 0.7 }}
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <MotionSection className="text-center mb-16">
          ...
          <h2 className="font-[family-name:var(--font-space-grotesk)] text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-4">
            Your Intelligence <span className="text-gradient">Command Center</span>
          </h2>
          ...
          <div className="glass rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-mono text-white/25 ml-2">helix-dashboard</span>
              </div>
              <div className="flex items-center gap-4">
                <motion.div
                  className="flex items-center gap-1.5 text-xs text-[#00D4AA]"
                ...
                <span className="text-xs font-mono text-white/25">{liveTime}</span>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {portfolioMetrics.map((m, i) => (
                  <StaggerItem key={i}>
                    <motion.div
                      whileHover={{ scale: 1.03, borderColor: "rgba(91,140,255,0.2)" }}
                      className="glass rounded-xl p-4 transition-colors"
                    >
                      <div className="text-xs text-white/25 mb-1">{m.label}</div>
                      <div className="text-xl font-bold font-[family-name:var(--font-space-grotesk)]">{m.value}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="glass rounded-xl p-4">
                  ...
                  <StaggerContainer className="space-y-3 max-h-40 overflow-y-auto">
                        ...
                        <div className="flex items-start gap-2">
                          {a.status === "success" ? (
                            ...
                            <motion.div
                              ...
                              className="w-3 h-3 rounded-full border border-[#F59E0B] border-t-transparent mt-0.5 shrink-0"
                            />
                          )}
                          <div className="min-w-0">
                            <div className="text-xs text-white/55 truncate">{a.agent}</div>
                            <div className="text-[10px] text-white/25">{a.action}</div>
                          </div>
                        </div>
                      </StaggerItem>
                    ))}
                    ...
                    <div key={i} className="flex items-center gap-3">
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-white/55">{w.name}</span>
                          <span className="text-[10px] font-mono text-white/25">{w.progress}%</span>
                        </div>
                        <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                          <motion.div
                            ...
                            style={{ backgroundColor: w.progress === 100 ? "#00D4AA" : "#5B8CFF" }}
                          />
                        </div>
                      </div>
